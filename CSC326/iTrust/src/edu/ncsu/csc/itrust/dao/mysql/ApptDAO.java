...
public class ApptDAO {
	...
	public List<ApptBean> getAppt(final int apptID) throws SQLException, DBException {
		try(Connection conn = factory.getConnection();
				...
				PreparedStatement pstring = conn.prepareStatement("SELECT * FROM appointment WHERE appt_id=?");
			) {
	...
	public List<ApptBean> getApptsFor(final long mid) throws SQLException, DBException {
		...
		if(mid >= MIN_MID){
			try(PreparedStatement pstring = conn.prepareStatement("SELECT * FROM appointment WHERE doctor_id=? AND sched_date > NOW() ORDER BY sched_date;")){
				pstring.setLong(1, mid);
				results = pstring.executeQuery();
			};
		}
		else {
			try(PreparedStatement pstring = conn.prepareStatement("SELECT * FROM appointment WHERE patient_id=? AND sched_date > NOW() ORDER BY sched_date;")){
				pstring.setLong(1, mid);
				results = pstring.executeQuery();
			};
		}
	...
	public List<ApptBean> getAllApptsFor(long mid) throws SQLException, DBException {
		...
		if(mid >= MIN_MID){
			try (PreparedStatement pstring = conn.prepareStatement("SELECT * FROM appointment WHERE doctor_id=? ORDER BY sched_date;");){
				pstring.setLong(1, mid);
				results = pstring.executeQuery();
			}
		}
		else {
			try(PreparedStatement pstring = conn.prepareStatement("SELECT * FROM appointment WHERE patient_id=? ORDER BY sched_date;");){
				pstring.setLong(1, mid);
	...
	public void scheduleAppt(final ApptBean appt) throws SQLException, DBException {
		...
		try(PreparedStatement pstring1 = this.abloader.loadParameters(pstring, appt);){
	...
	public void removeAppt(final ApptBean appt) throws SQLException, DBException {
		try(Connection conn = factory.getConnection();
				...
				PreparedStatement pstring = conn.prepareStatement("DELETE FROM appointment WHERE appt_id=?");
){
	...
	public List<ApptBean> getAllHCPConflictsForAppt(final long mid, final ApptBean appt) throws SQLException, DBException{
		...
		try(Connection conn = factory.getConnection();
				PreparedStatement pstring = conn.prepareStatement("SELECT * " +
	...
	public List<ApptBean> getAllConflictsForDoctor(final long mid) throws SQLException, DBException{
		try(Connection conn = factory.getConnection();
			...
			PreparedStatement	pstring = conn.prepareStatement("SELECT a1.* " +
