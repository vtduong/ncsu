...
public class CDCWeightStatsDAO extends CDCStatsDAO {
	...
	public void storeStats(CDCStatsBean statsBean) throws DBException {
		try (Connection conn = factory.getConnection();
			PreparedStatement ps = conn.prepareStatement("INSERT INTO cdcweightstats (sex, age, L, M, S) VALUES (?,?,?,?,?) "
	...
	public CDCStatsBean getCDCStats(int sex, float age) throws DBException {
		...
		try (Connection conn = factory.getConnection();
			PreparedStatement ps = conn.prepareStatement("SELECT * FROM cdcweightstats WHERE sex=? AND age=?");
			){
			ps.setInt(1, sex);
			ps.setFloat(2, age);
