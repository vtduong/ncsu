...
public class AuthDAO {
	...
	public void setSecurityQuestionAnswer(final String question, final String answer, final long mid) throws DBException {
		try(Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("UPDATE users SET sQuestion = ?, sAnswer = ? WHERE MID = ?");
				) {
	...
	public Role getUserRole(final long mid) throws ITrustException {
		try(Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("SELECT role FROM users WHERE MID=?");
				) {
			pstmt.setLong(1, mid);
	...
	public boolean getDeactivated(final long mid) throws ITrustException{
		...
		if(role.equals(Role.PATIENT)) {
			try(Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("SELECT DateOfDeactivation FROM patients WHERE MID=?");
				) {
				pstmt.setLong(1, mid);
				ResultSet results;
				results = pstmt.executeQuery();
	...
	public void resetPassword(final long mid, final String password) throws DBException {
		try (Connection conn = factory.getConnection();
			PreparedStatement pstmt = conn.prepareStatement("UPDATE users SET password=?, salt=? WHERE MID=?");
			){
			String salt = shakeSalt();
	...
	public String getSecurityQuestion(final long mid) throws ITrustException {
		try(Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("SELECT sQuestion FROM users WHERE MID=?");
				) {
			pstmt.setLong(1, mid);
	...
	public String getSecurityAnswer(final long mid) throws ITrustException {
		try (Connection conn = factory.getConnection();
			PreparedStatement pstmt = conn.prepareStatement("SELECT sAnswer FROM users WHERE MID=?");
			){
			pstmt.setLong(1, mid);
			final ResultSet results = pstmt.executeQuery();
			if (results.next()){
	...
	public int getResetPasswordFailures(final String ipAddr) throws DBException {
		int numFailures;
		try (Connection conn = factory.getConnection();
			PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM resetpasswordfailures WHERE IPADDRESS=?");
			){
			pstmt.setString(1, ipAddr);
			final ResultSet results = pstmt.executeQuery();
			if (results.next()) {
	...
	public int getLoginFailures(final String ipAddr) throws DBException {
		int numFailures;
		try (Connection conn = factory.getConnection();
			PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM loginfailures WHERE IPADDRESS=?");
			){
			pstmt.setString(1, ipAddr);
			final ResultSet results = pstmt.executeQuery();
			if (results.next()) {
	...
	private void insertLoginFailureRow(final String ipAddr, int failureCount) throws DBException, SQLException {
		try (Connection conn = factory.getConnection();
				PreparedStatement pstmt = conn.prepareStatement("INSERT INTO loginfailures(IPAddress, failureCount) VALUES(?,?)");
			){ 
			pstmt.setString(1, ipAddr);
	...
	private void insertResetPasswordRow(final String ipAddr, final int failureCount) throws DBException, SQLException {
		try (Connection conn = factory.getConnection();
			PreparedStatement pstmt = conn.prepareStatement("INSERT INTO resetpasswordfailures(IPAddress, failureCount) VALUES(?,?)");
			){ 
			pstmt.setString(1, ipAddr);
			pstmt.setInt(2, failureCount);
			pstmt.executeUpdate();
	...
	private void updateFailuresToZero(final String ipAddr) throws DBException, SQLException {
		try (Connection conn = factory.getConnection();
			PreparedStatement pstmt = conn.prepareStatement("UPDATE loginfailures SET failureCount=0 WHERE IPAddress=?");
			){
			pstmt.setString(1, ipAddr);
			pstmt.executeUpdate();
	...
	private void updateResetFailuresToZero(final String ipAddr) throws DBException, SQLException {
		try(Connection conn = factory.getConnection(); 
			PreparedStatement pstmt = conn.prepareStatement("UPDATE resetpasswordfailures SET failureCount=0 WHERE IPAddress=?");
			){
			pstmt.setString(1, ipAddr);
			pstmt.executeUpdate();
	...
	public boolean checkUserExists(final long mid) throws DBException {
		try (Connection conn = factory.getConnection();
			PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM users WHERE MID=?");
			){
			pstmt.setLong(1, mid);
			final ResultSet results = pstmt.executeQuery();
	...
	public boolean authenticatePassword(final long mid, final String password) throws DBException {
		try (Connection conn = factory.getConnection();
			PreparedStatement pstmt = conn.prepareStatement("Select * FROM users WHERE MID=? AND password=?");
			){
	...
	public void setDependent(long mid, boolean dependency) throws DBException {
		try (Connection conn = factory.getConnection();
			PreparedStatement ps = conn.prepareStatement("UPDATE users SET isDependent=? WHERE MID=?");
			){
	...
	public boolean isDependent(final long mid) throws DBException {
		try (Connection conn = factory.getConnection();
			PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM users WHERE MID=? AND isDependent=1");
			){
			pstmt.setLong(1, mid);
			final ResultSet results = pstmt.executeQuery();
			final boolean check = results.next();
	...
	public String getSalt(long mid){
		...
		ResultSet rs = null;
		try (Connection conn = factory.getConnection();
			PreparedStatement ps = conn.prepareStatement("SELECT Salt FROM users WHERE MID=?");
			){
			ps.setLong(1, mid);
			rs = ps.executeQuery();
			if(rs.next()){
