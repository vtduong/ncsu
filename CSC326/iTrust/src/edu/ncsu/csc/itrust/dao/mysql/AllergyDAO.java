...
public class AllergyDAO {
	...
	public List<AllergyBean> getAllergies(final long pid) throws DBException {
		...
		try(Connection conn = factory.getConnection();
			PreparedStatement pstring = conn.prepareStatement("SELECT * FROM allergies WHERE PatientID=? ORDER BY FirstFound DESC");) {
	...
	public void addAllergy(final AllergyBean allergy) throws DBException {
		...
		try(Connection conn = factory.getConnection();
				PreparedStatement pstring = conn.prepareStatement("INSERT INTO allergies(PatientID, Code, FirstFound, Description) VALUES (?,?,?,?)");) {
