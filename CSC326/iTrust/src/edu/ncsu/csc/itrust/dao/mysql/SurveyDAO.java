...
public class SurveyDAO {
	...
	public void addCompletedSurvey(SurveyBean surveyBean, Date date) throws DBException {
		...
		try {
			if (surveyBean == null){
				throw new SQLException("Exception thrown from Evil Test Connection Driver");
			}
			if ( surveyBean.getWaitingRoomMinutes() <= 0 || surveyBean.getWaitingRoomMinutes() > 999 ) {
				throw new SQLException("Waiting Room Minutes must be 1-999");
			}
			if ( surveyBean.getExamRoomMinutes() <= 0 || surveyBean.getExamRoomMinutes() > 999 ) {
				throw new SQLException("Exam Room Minutes must be 1-999");
			}
