...
public class DiagnosesDAO {
	...
	public List<DiagnosisBean> getList(long visitID) throws DBException {
		try (Connection conn = factory.getConnection();
			PreparedStatement ps = conn.prepareStatement("Select * From ovdiagnosis,icdcodes Where ovdiagnosis.VisitID = ? "
	...
	public DiagnosisStatisticsBean getDiagnosisCounts(String icdCode, String zipCode, java.util.Date lower, java.util.Date upper) throws DBException {
		...
		try (Connection conn = factory.getConnection();
			PreparedStatement ps = conn.prepareStatement("SELECT * FROM ovdiagnosis INNER JOIN officevisits ON ovdiagnosis.VisitID=officevisits.ID INNER JOIN patients ON officevisits.PatientID=patients.MID WHERE ICDCode=? AND zip=? AND visitDate >= ? AND visitDate <= ? ");
			){
			...
			try (PreparedStatement ps1 = conn.prepareStatement("SELECT * FROM ovdiagnosis INNER JOIN officevisits ON ovdiagnosis.VisitID=officevisits.ID INNER JOIN patients ON officevisits.PatientID=patients.MID WHERE ICDCode=? AND zip LIKE ? AND visitDate >= ? AND visitDate <= ? ");){
				ps1.setString(1, icdCode);
				ps1.setString(2, zipCode.substring(0, 3) + "%");
				ps1.setTimestamp(3, new Timestamp(lower.getTime()));
				// add 1 day's worth to include the upper
				ps1.setTimestamp(4, new Timestamp(upper.getTime() + 1000L * 60L * 60 * 24L));
				rs = ps1.executeQuery();
	...
	public Date findEarliestIncident(String icdCode) throws DBException{
		try (Connection conn = factory.getConnection();
			PreparedStatement ps = conn.prepareStatement("SELECT MIN(visitDate) AS visitDate FROM ovdiagnosis d INNER JOIN officevisits o " +
										" ON d.VisitID=o.ID " + 
										" WHERE ICDCode LIKE ?");
			){
			ps.setString(1, icdCode + "%");
	...
	public void remove(long ovDiagnosisID) throws DBException {
		try (Connection conn = factory.getConnection();
			PreparedStatement ps = conn.prepareStatement("DELETE FROM ovdiagnosis WHERE ID=? ");
			){
