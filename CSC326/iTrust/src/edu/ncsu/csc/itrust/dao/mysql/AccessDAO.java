...
//		PreparedStatement ps = null;
		try ( Connection conn = factory.getConnection();
			  PreparedStatement ps = conn.prepareStatement("SELECT Value FROM globalvariables WHERE Name='Timeout'")
			)
		{	
...
//		}
	...
	public void setSessionTimeoutMins(int mins) throws DBException {
		...
		try (Connection conn = factory.getConnection();
			PreparedStatement ps = conn.prepareStatement("UPDATE globalvariables SET Value=? WHERE Name='Timeout'");){
	...
	private void insertDefaultTimeout( final int mins) throws SQLException, DBException {
		...
		try(Connection conn = factory.getConnection();
				PreparedStatement pstring = conn.prepareStatement("INSERT INTO globalvariables(Name,Value) VALUES ('Timeout', ?)");
				){
