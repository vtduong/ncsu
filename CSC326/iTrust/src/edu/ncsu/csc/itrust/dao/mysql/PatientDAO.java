...
//		PreparedStatement ps = null;
		try (
			...
			PreparedStatement ps1 = conn.prepareStatement("UPDATE users U, representatives R SET U.isDependent=0 WHERE R.representerMID=? AND "
					+ "R.representeeMID=U.MID AND R.representeeMID NOT IN "
					+ "(SELECT representeeMID FROM representatives WHERE representerMID<>?)");
			PreparedStatement ps2= conn.prepareStatement("DELETE FROM representatives WHERE representerMID=?");

			)
		{
			ps1.setLong(1, representerMID);
			ps1.setLong(2, representerMID);
			ps1.executeUpdate();
			ps2.setLong(1, representerMID);
			ps2.executeUpdate();
		} catch (SQLException e) {
...
//		PreparedStatement ps = null;
		try (
			Connection conn = factory.getConnection();
			PreparedStatement ps1 = conn.prepareStatement("DELETE FROM representatives WHERE representeeMID=?");
			PreparedStatement ps = conn.prepareStatement("UPDATE users SET isDependent=0 WHERE MID=?");

			)
		{
			ps1.setLong(1, representeeMID);
			ps1.executeUpdate();
			//ps.close();
