...
<%
SurveyAction action = new SurveyAction(DAOFactory.getProductionInstance());
...
SurveyDAO survey = new SurveyDAO(DAOFactory.getProductionInstance());

//SurveyDAO fillTest = new SurveyDAO( DAOFactory.getProductionInstance() );
//boolean formIsFilled = fillTest.isSurveyCompleted( visitID );
...
if(formIsFilled) {
	...
	//add waiting minutes condition which doesn't accept 0
    ...
    try {
		...
		response.sendRedirect("viewMyRecords.jsp?message=Survey%20Successfully%20Submitted");
    	 ...
    	 %><span style="color: #cc0000; font-size: 20px; font-weight: bold;"><%=StringEscapeUtils.escapeHtml(e.getExtendedMessage())%></span><%
	}
} else{
...
<%
if ( !survey.isSurveyCompleted( visitID ) ) {
%>
<div id=Header>
<h1>iTrust Patient Survey for Office Visit on <%= StringEscapeUtils.escapeHtml("" + (visitDateStr )) %> </h1>
</div>
<div id=Content>

<form action="survey.jsp" method="post" name="mainForm">
<input type="hidden" name="formIsFilled" value="true">
<input type="hidden" name="ovDate" value="<%= StringEscapeUtils.escapeHtml("" + (visitDateStr)) %>"> 
...
%>
	<div id=Header>
		<h1 id="surveyRepeatErrorMessage">
